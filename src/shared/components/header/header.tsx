    ...
    <Navbar expand="lg" className={`header-container sticky-top shadow ${props.hide && 'd-none'}`}>
      ...
      <Collapse isOpen={isOpen} navbar>
        <Nav className="mr-auto" navbar>
          <Link className="header-option" to="/como-funciona" onClick={mobile ? toggle : () => {}} style={{ textDecoration: 'none' }}>
            <h6>COMO FUNCIONA</h6>
          </Link>
          <Link className="header-option" to="/valores" onClick={mobile ? toggle : () => {}} style={{ textDecoration: 'none' }}>
            <h6>VALORES</h6>
          </Link>
          <Link className="header-option" to="/preavaliacao" onClick={mobile ? toggle : () => {}} style={{ textDecoration: 'none' }}>
            <h6>PRÉ-AVALIAÇÃO</h6>
          </Link>
        </Nav>
        <Col>
          <Button tag={Link} to="/preavaliacao" onClick={mobile ? toggle : () => {}} className={`general-button ${mobile ? '' : 'float-right'}`}>
